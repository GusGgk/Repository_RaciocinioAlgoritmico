...
carros = ["Fusca","Gol","HB20","Opala","Civic"]
km_por_litro = [7,10,8,12,9]
...
for i in range(5):
    nome = carros[i]
    consumo = km_por_litro[i]
    litros = 1000 / consumo
    custo = litros * 2.25
    print(f"{i+1} - {nome.lower()} - {consumo:.1f} - {litros:.1f} litros - R$ {custo:.2f}")

mais_economico = max(km_por_litro)
indiceEconomico = km_por_litro.index(mais_economico)
carro_enconomico = carros[indiceEconomico]
print(f"O menor consumo é do: {carro_enconomico}")    
